...
def main():
    ...
    def askopenfile():
        filename, file_path = tk.filedialog.askopenfilenames(
        ...
        if variable.get() == 'English':
            print(filename)
            start_conversion = tk.ttk.Button(
                root,
                text='Start conversion.',
                command=lambda: PyRead.Utils.img_to_text(
                    obj, filename, file_path, 'eng'))
            ...
            start_conversion = tk.ttk.Button(
                ...
                text='Start conversion.',
                command=lambda: PyRead.Utils.img_to_text(
                    obj, filename, file_path, 'chi_sim'))
            start_conversion.pack(side=tk.BOTTOM)

    choose_file = tk.ttk.Button(big_frame,
